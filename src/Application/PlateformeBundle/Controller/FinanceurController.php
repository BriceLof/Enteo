...
class FinanceurController extends Controller
{
    ...
    public function listeOpcaOpacifAction(Request $request, $nom){
        if ($request->isXmlHttpRequest()) {
            if($nom == 'OPCA'){
                $results = array('','ACTALIANS','AFDAS','AGECIF CAMA','AGEFICE','AGEFOS-PME','ANFA','ANFH','CONSTRUCTYS', 'FAFCEA', 'FAFIEC','FAFIH','FAFSEA','FAF.TT','FIF PL','FORCO','INTERGROS','OPCAIM ADEFIM','OPCALIA','OPCALIM','OPCA BAIA','OPCA CGM','OPCA DEFI','OPCA de la contruction','OPCA PL','OPCA Transports','OPCA 3+','UNAGECIF','UNIFAF','UNIFORMATION');
